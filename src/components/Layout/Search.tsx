...
export const SearchBar: React.FC<{ text: string, onChange: React.ChangeEventHandler<HTMLInputElement> }> = ({ text, onChange }) => {
    return (
        <div className="flex items-center gap-2 border-2 border-red-300 max-w-[700px] m-auto my-8 h-auto pl-3 rounded-sm">
            ...
            <input
                ...
                onChange={onChange}
            />
            <button className="bg-red-600 border-none font-semibold rounded-sm h-10 w-24 text-center cursor-pointer text-white ease-in duration-300 hover:tracking-[0.1rem]">
                <a href="#">Tìm kiếm</a>
            </button>
        </div>
    )
